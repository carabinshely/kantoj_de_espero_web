...
for (const marker of ['data-listening-surface', 'data-listening-provider', 'data-listening-fallback', 'data-listening-state', 'data-listening-entity-id', 'data-listening-spotify-embed']) {
...
if (!component.includes('data-loaded-message') || component.includes('message.remove()')) fail({ check: 'verify:listening', problem: 'Loaded provider state hides its status message.', cause: 'Provider status must remain visible through loading, loaded, switch, and error states.', path: 'src/components/ListeningSurface.astro', fix: 'Keep a loaded message in the panel instead of removing it after iframe load.' });
if (!component.includes('if (!panel.contains(message)) panel.prepend(message)')) fail({ check: 'verify:listening', problem: 'Listening status disappears after a provider switch.', cause: 'A loaded iframe removes its status node, so later state updates must restore it.', path: 'src/components/ListeningSurface.astro', fix: 'Reinsert the status message from setState before updating it.' });
...
for (const path of [`/en/playlists/${start.slug_en}/`, `/eo/ludlistoj/${start.slug_eo}/`]) {
  const html = read(route(path));
  for (const marker of ['data-listening-mode="spotify-embed"', 'data-listening-spotify-embed', 'https://open.spotify.com/embed/playlist/5rbdelkCrs26Tjc8y2gqWD']) if (!html.includes(marker)) fail({ check: 'verify:listening', problem: `Start Here route lacks ${marker}.`, cause: 'The Spotify-only playlist must render its player directly.', path: route(path), fix: 'Render the Spotify playlist iframe without service-selection or load controls.' });
  for (const marker of ['data-listening-load', 'Choose a service to listen', 'Elektu servon por aŭskulti']) if (html.includes(marker)) fail({ check: 'verify:listening', problem: `Start Here route still renders ${marker}.`, cause: 'A Spotify-only playlist does not need provider-selection or load controls.', path: route(path), fix: 'Use the direct Spotify playlist embed branch.' });
}
...
pass('verify:listening', `provider URL validation (${providerFixtures.length} positive/negative fixtures), compact menus, truthful lazy song-player states, keyboard tabs, external-only Apple fallback, bilingual routes, and direct Spotify playlist embedding verified`);
