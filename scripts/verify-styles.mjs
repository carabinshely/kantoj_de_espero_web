...
for (const removed of ['.hero::after']) {
...
for (const required of ['data-theme="dark"', '.theme-toggle']) {
  if (!css.includes(required)) fail({ check: 'verify:styles', problem: `Missing prototype-inspired reading control: ${required}`, cause: 'The compact light/dark control is a required part of the approved header treatment.', path: 'src/styles/global.css', fix: 'Restore the compact bordered theme control and its dark token set.' });
}
const themes = {
  ...
  dark: { ink: '#F4EFE6', paper: '#172126', surface: '#202D31', coral: '#F07A68', buttonAccentInk: '#172126', coralText: '#F48A79', greenText: '#78B9A8', sky: '#8ED4DF', mutedText: '#C4C9C2' },
};
const contrastPairs = [
  ['body text', 'ink', 'paper'],
...
pass('verify:styles', `all small text/control contrast pairs meet WCAG AA (${results.join('; ')}); prototype-inspired compact reading control and five playlist moods are present`);
